...
function ImprimirConteudo() {
 ...
 /* Exemplo:  
    ...
    let atividadeEscolhida = localStorage.getItem("tipoAtividadeFisica");
    ...
    for (let i = 1; i < objDados.conteudo.length; i++){
        if (objDados.conteudo[i].categoria.id == "exFisico" && objDados.conteudo[i].categoria.subCategoria.id2 == atividadeEscolhida) {
            {
            ...
            <div class="card-body">
              ...
              <a href="AtividadeFisica-Individual.html" class="btn btn-primary" onclick="localStorage.setItem('escolhaConteudoindividual', ${i});">Clique aqui para ser redirecionado</a>
            </div>
          </div>`;
