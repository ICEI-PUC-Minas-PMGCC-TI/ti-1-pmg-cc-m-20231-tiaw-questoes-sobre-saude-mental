...
function ImprimirConteudo() {
 ...
 /* Exemplo:  
    ...
    for (let i = 1; i < objDados.conteudo.length; i++){
        if (objDados.conteudo[i].categoria.id == "exMeditacao" && objDados.conteudo[i].categoria.subCategoria.id2 == atividadeEscolhida) {
            {
            ...
            <div class="card-body">
              ...
              <a href="AtividadeMeditacao-Individual.html" class="btn btn-primary" onclick="localStorage.setItem('escolhaConteudoindividual', ${i});">Clique aqui para ser redirecionado</a>
            </div>
          </div>`;
